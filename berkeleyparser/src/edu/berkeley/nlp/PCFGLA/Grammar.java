...
public class Grammar implements java.io.Serializable {
		...
		for (int i = 0; i < closedSumRulesWithChild[pState].size(); i++) {
			...
			for (int j = 0; j < closedSumRulesWithParent[cState].size(); j++) {
				...
				UnaryRule cr = (UnaryRule) closedSumRulesWithParent[cState]
						.get(j);
				UnaryRule resultR = new UnaryRule(parentState, cr
						.getChildState());
	...
	public double[][][] getBinaryScore(short pState, short lState, short rState) {
		...
		if (r != null)
			return r.getScores2();
		if (GrammarTrainer.VERBOSE)
			System.out.println("The requested rule (" + bSearchRule
					+ ") is not in the grammar!");
	...
	public void printSymbolCounter(Numberer tagNumberer) {
		...
		for (Integer i : set) {
			pq
					.add((String) tagNumberer.object(i), symbolCounter
							.getCount(i, 0));
		...
		for (int tag = 0; tag < splitTrees.length; tag++) {
			...
			for (Tree<Short> leaf : splitTree.getTerminals()) {
				...
				if (numSubStates[tag] > oldNumSubStates[tag]) {
					children
							.add(new Tree<Short>((short) (2 * leaf.getLabel())));
	...
	public void overwriteWithMaxent() {
		//this is the map which maps each subrule to its location in binary rule
		HashMap<String, Pair<BinaryRule, Triple>> str2brule = new HashMap<String, Pair<BinaryRule,Triple>>();
		
		//create a eventstream
		BinaryRuleEventStream binaryRuleEventStream = new BinaryRuleEventStream();
		
		for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
			double[][][] score2 = binaryRuleCounter.getCount(binaryRule);
			for (Triple triple : binaryRule.getAllSubRules(score2)) {
				String bruleStr = binaryRule.getStrSubRule(triple);
				
				str2brule.put(bruleStr, new Pair<BinaryRule, Triple>(binaryRule, triple));
				
				for (int i=0; i<Math.round(1+binaryRule.getCountForSubRule(score2, triple)); i++) {
					binaryRuleEventStream.addEvent(bruleStr, BinaryRuleEventStream.getContext(bruleStr));
				}
				
				//reset this count to zero
				binaryRule.setProbForSubRule(score2, triple, 0.0);
			}
		}
		
		//now train the maxent model
		GISModel bruleMaxentModel = GIS.trainModel(binaryRuleEventStream, 10, 0);
		...
		for (BinaryRule binaryRule : binaryRuleCounter.keySet()) {
			...
			for (Triple triple : binaryRule.getAllSubRules(score2)) {
				...
				double[] outcomes = bruleMaxentModel.eval(BinaryRuleEventStream.getContext(bruleStr));
				
				for (int i = 0; i < outcomes.length; i++) {
					Pair<BinaryRule, Triple> pair = str2brule.get(bruleMaxentModel.getOutcome(i));
				
					//select the outcomes which match this binaryrule (ignoring all others)
					pair.getFirst().incProbForSubRule(score2, pair.getSecond(), outcomes[i]);
				}
			}
		}		
		
		//this is the map which maps each subrule to its location in binary rule
		HashMap<String, Pair<UnaryRule, Pair<Integer, Integer>>> str2urule = new HashMap<String, Pair<UnaryRule,Pair<Integer, Integer>>>();
		
		//create a eventstream
		UnaryRuleEventStream unaryRuleEventStream = new UnaryRuleEventStream();
		
		for (UnaryRule unaryRule : unaryRuleCounter.keySet()) {
			double[][] scores2 = unaryRuleCounter.getCount(unaryRule);
			for (Pair<Integer, Integer> pair : unaryRule.getAllSubRules(scores2)) {
				String uruleStr = unaryRule.getStrSubRule(pair);
				
				str2urule.put(uruleStr, new Pair<UnaryRule, Pair<Integer, Integer>>(unaryRule, pair));
				
				for (int i=0; i<Math.round(1+unaryRule.getCountForSubRule(scores2, pair)); i++) {
					unaryRuleEventStream.addEvent(uruleStr, UnaryRuleEventStream.getContext(uruleStr));
				}
				
				//reset this count to zero
				unaryRule.setProbForSubRule(scores2, pair, 0.0);
			}
		}
		
		//now train the maxent model
		GISModel uruleMaxentModel = GIS.trainModel(unaryRuleEventStream);
		...
		for (UnaryRule unaryRule : unaryRuleCounter.keySet()) {
			...
			for (Pair<Integer, Integer> pair : unaryRule.getAllSubRules(scores2)) {
				...
				double[] outcomes = uruleMaxentModel.eval(UnaryRuleEventStream.getContext(uruleStr));
				
				for (int i = 0; i < outcomes.length; i++) {
					Pair<UnaryRule, Pair<Integer, Integer>> pair2 = str2urule.get(uruleMaxentModel.getOutcome(i));
					
					//select the outcomes which match this binaryrule (ignoring all others)
					pair2.getFirst().incProbForSubRule(scores2, pair2.getSecond(), outcomes[i]);
				}
			}
		}		
	}
